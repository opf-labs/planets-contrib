...
public class FFMpegMigration implements Migrate, Serializable {
    ...
    public MigrateResult migrate(final DigitalObject digitalObject,
        ...
        try {
            ...
            String strRsc = "/eu/planets_project/services/migration/ffmpeg/ffmpeg.properties";
        ...
        FFMpegFormat in = null;
        try {
            in = FFMpegFormat.loopup(inputFormat);
        } catch (NoSuchFormatException e) {
            report.setError("The input format is not on the known list");
            return fail(report);
        }
        FFMpegFormat out = null;
        try {
            out = FFMpegFormat.loopup(outputFormat);
        } catch (NoSuchFormatException e) {
        ...
        try {
            migrationPath = FFMpegMigrationPath.lookup(in,out).getTool();
    ...
    public ServiceDescription describe() {
        ...
        FFMpegMigrationPath[] mypaths = FFMpegMigrationPath.values();
        MigrationPath[] paths = new MigrationPath[mypaths.length];
        int i=0;
        for (FFMpegMigrationPath mypath: mypaths){
